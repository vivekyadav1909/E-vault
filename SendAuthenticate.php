...
if ($cat == "Images") {
    ...
    if ((($_FILES["txtUpload"]["type"] == "image/gif") || ($_FILES["txtUpload"]["type"] == "image/jpeg") || ($_FILES["txtUpload"]["type"] == "image/jpg") || ($_FILES["txtUpload"]["type"] == "image/pjpeg") || ($_FILES["txtUpload"]["type"] == "image/x-png") || ($_FILES["txtUpload"]["type"] == "image/png") || ($_FILES["txtUpload"]["type"] == "image/bmp")) && in_array($extension, $allowedExts)) {
        ...
        if ($_FILES["txtUpload"]["error"] > 0) {
            ...
            move_uploaded_file($_FILES["txtUpload"]["tmp_name"], "users/$touser/images/" . $_FILES["txtUpload"]["name"]);
            ...
            $mail->AddAttachment("users/$touser/images/" . $filename);

            if (!$mail->Send()) {

                echo "<script type=text/javascript>";
                echo "if(confirm('Unable to sent Image file'))";
                echo "{";
                echo "location.href='Account.php?upload=false";
                echo "}";
                echo "</script>";
        ...
        echo "<script type=text/javascript>";
        echo "if(confirm('File Format is not valid ...'))";
        echo "{";
        echo "location.href='Account.php?upload=false'";
        echo "}";
        echo "</script>";
...
if ($cat == "Videos") {
    ...
    if (in_array($extension, $allowedExts)) {
        ...
        if ($_FILES["txtUpload"]["error"] > 0) {
            ...
            move_uploaded_file($_FILES["txtUpload"]["tmp_name"], "users/$touser/videos/" . $_FILES["txtUpload"]["name"]);
            $filename = $_FILES["txtUpload"]["name"];
            $mail = new PHPMailer();
            ...
            $mail->AddAttachment("users/$touser/videos/" . $filename);

            if (!$mail->Send()) {

                echo "<script type=text/javascript>";
                echo "if(confirm('Unable to sent Video file'))";
                echo "{";
                echo "location.href='Account.php?upload=false'";
                echo "}";
                echo "</script>";
...
if ($cat == "Documents") {
    ...
    if (in_array($extension, $allowedExts)) {
        if ($_FILES["txtUpload"]["error"] > 0) {
            ...
            move_uploaded_file($_FILES["txtUpload"]["tmp_name"], "users/$touser/documents/" . $_FILES["txtUpload"]["name"]);
            $filename = $_FILES["txtUpload"]["name"];
            $mail = new PHPMailer();
            ...
            $mail->AddAttachment("users/$touser/documents/" . $filename);
